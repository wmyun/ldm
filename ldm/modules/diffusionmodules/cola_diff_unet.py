...
from ldm.modules.diffusionmodules.utils import  (
    to_cuda, maybe_to_torch, no_op, 
    InitWeights_He, softmax_helper, sigmoid_helper)
...
class NewnetModel(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            ...
            self.hwm = Generic_Newnet(int(self.in_channels/2), features, 1, 5)
    ...
    def forward(self, x, timesteps, y=None, pseudo=None):
        """
        ...
        uemb, cal = self.highway_forward(pseudo, h.shape[1])#, [hs[2],hs[5],hs[8],hs[11]])
        h = h + uemb
...
class Generic_Newnet(SynthesisNetwork):
    ...
    def forward(self, x, emb_out_channel):#, hs):
        skips = []
        rep_outputs = []
        ...
        emb = conv_nd(2, x.size(1), emb_out_channel, 1).to(device = x.device)(x)
        print(emb.shape)
